...
module ScdTest
  ...
  class ScdType1Test < Test::Unit::TestCase
    include ScdTestHelpers
      
    def test_run_1_inserts_record
      do_run(1)
      assert_equal 1, count_bobs
    end
  
    def test_run_1_sets_original_address
      do_run(1)
      assert_boston_address(find_bobs.first)
    end
  
    def test_run_1_sets_original_id
      do_run(1)
      assert_equal 1, find_bobs.first.id
    end
  
    def test_run_2_deletes_old_record
      do_run(1)
      do_run(2)
      assert_equal 1, count_bobs, "new record created, but old not deleted: #{find_bobs.inspect}"
    end
  
    def test_run_2_updates_address
      do_run(1)
      do_run(2)
    ...
    def test_run_2_keeps_id
      do_run(1)
      do_run(2)
      assert_equal 1, find_bobs.first.id
    end
  
    def test_no_change_skips_load
      do_run(1)
      do_run(1)
    ...
    def test_change_once_only_loaded_once
      do_run(1)
      do_run(2)
      do_run(2)
      assert_equal 1, count_bobs
    ...
    def test_revert_udpates_address_on_new_record
      do_run(1)
      do_run(2)
      do_run(1)
      assert_boston_address(find_bobs.first)
  ...
  class ScdType2Test < Test::Unit::TestCase
    include ScdTestHelpers
    ...
    def test_run_1_sets_effective_date
      do_run(1)
    ...
    def test_run_1_sets_end_date
      do_run(1)
      assert_equal @end_of_time, find_bobs.first.end_date
    end
  
    def test_run_1_sets_latest_version
      do_run(1)
      assert find_bobs.first.latest_version?
    end
  
    def test_run_2_inserts_new_record
      do_run(1)
      do_run(2)
      assert_equal 2, count_bobs
    end
  
    def test_run_2_keeps_primary_key_of_original_version
      do_run(1)
      do_run(2)
      assert_not_nil find_bobs.detect { |bob| 1 == bob.id }
    end
  
    def test_run_2_increments_primary_key_for_new_version
      do_run(1)
      do_run(2)
      assert_not_nil find_bobs.detect { |bob| 2 == bob.id }
    end
  
    def test_run_2_expires_old_record
      do_run(1)
      do_run(2)
    ...
    def test_run_2_keeps_address_for_expired_record
      do_run(1)
      do_run(2)
    ...
    def test_run_2_updates_address_on_new_record    
      do_run(1)
      do_run(2)
    ...
    def test_run_2_activates_new_record
      do_run(1)
      do_run(2)
    ...
    def test_run_2_sets_end_date_for_new_record
      do_run(1)
      do_run(2)
      assert_equal @end_of_time, find_bobs.detect { |bob| 2 == bob.id }.end_date
    end
  
    def test_run_2_shifts_latest_version
      do_run(1)
      do_run(2)
    ...
    def test_revert_inserts_new_record
      do_run(1)
      do_run(2)
      do_run(1)
    ...
    def test_scd_change_deletes_only_one_row
      do_run(1) # put first version in
      do_run(2) # put second version in
      # Two records right now
      assert_equal 2, count_bobs
      do_run(1) # put third version in (same as first version, but that's irrelevant)
      # was failing because first and second versions were being deleted.
      assert_equal 3, count_bobs
    end
  
    def test_non_scd_field_changes_dont_create_extra_record
      do_run_with_only_city_state_zip_scd(1)
      do_run_with_only_city_state_zip_scd(2)
      do_run_with_only_city_state_zip_scd(3)
      assert_equal 2, count_bobs
    end
  
    def test_non_scd_field_changes_keep_id
      do_run_with_only_city_state_zip_scd(1)
      do_run_with_only_city_state_zip_scd(2)
      do_run_with_only_city_state_zip_scd(3)
    ...
    def test_non_scd_field_changes_keep_dates
      do_run_with_only_city_state_zip_scd(1)
      do_run_with_only_city_state_zip_scd(2)
    ...
    def test_non_scd_field_changes_keep_latest_version
      do_run_with_only_city_state_zip_scd(1)
      do_run_with_only_city_state_zip_scd(2)
      do_run_with_only_city_state_zip_scd(3)
    ...
    def test_non_scd_fields_treated_like_type_1_fields
      do_run_with_only_city_state_zip_scd(1)
      do_run_with_only_city_state_zip_scd(2)
      do_run_with_only_city_state_zip_scd(3)
    ...
    def test_no_change_to_non_scd_fields_skips_load
      do_run_with_only_city_state_zip_scd(1)
      do_run_with_only_city_state_zip_scd(2)
      do_run_with_only_city_state_zip_scd(2)
      lines = lines_for('scd_test_type_2.txt')
    ...
    def do_run_with_only_city_state_zip_scd(run_num)
